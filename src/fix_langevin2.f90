...
contains
    subroutine init_fix_langevin2
        ...
        ! ===
    end subroutine init_fix_langevin2
    !
    ! --- langevin thermostat ---
    subroutine fix_langevin2
        use update_mod
        use commn
        implicit none
        integer j,i, idim, inum,imol
        double precision work_now(1:nlist), r83(3)
        double precision dt_wmi2gamma
        !
        if( .not. fflag ) return
        !
        fLbuf(:,:) = fL(:,:)
        do j=1,nlist
            dt_wmi2gamma = dt_wmi2(atlist(j))*gamma(j)
            do i=1,natseq(atlist(j))
                ...
                fL(:,imol) =  dimlist(:,j)*&
                    ( - gamma(j)*v(:,imol) + r83(:) &
                    - dt_wmi2gamma*(fbuf(:,imol)+f(:,imol)) )&
                    /(1d0 + dt_wmi2gamma)
                f(:,imol) = f(:,imol) + fL(:,imol)
                ! ---
            end do
        end do
    end subroutine fix_langevin2
    !
    ! --- compute culumtive enerty ---
    subroutine fix_langevin2_compute
        use update_mod
        use commn
        use constant_mod
        use fix_compute_mod
    ...
    end subroutine fix_langevin2_compute
end module fix_langevin2_mod
