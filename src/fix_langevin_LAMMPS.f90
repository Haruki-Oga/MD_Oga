...
module fix_langevin_LAMMPS_mod
    ...
    double precision,allocatable ::  fL(:,:),fLbuf(:,:),v2dt(:,:)
    !
contains
    subroutine init_fix_langevin_LAMMPS
        use update_mod
        use commn
        use file_mod
        use constant_mod
        ...
        sigR = [sigR,sigR_in]
        if(nlist==0)then
            allocate(fL(1:3,1:nmol),fLbuf(1:3,1:nmol),v2dt(1:3,1:nmol))
            fL(:,:) = 0d0
            fLbuf(:,:) = 0d0
            v2dt(:,:) = 0d0
        end if
        ...
        ! ===
    end subroutine init_fix_langevin_LAMMPS
    !
    ! --- langevin thermostat ---
    subroutine fix_langevin_LAMMPS
        use update_mod
        use commn
        implicit none
        ...
        do j=1,nlist
            do i=1,natseq(atlist(j))
                ...
                call random_number(r83)
                r83 = (r83 - 0.5d0)*sqrt(12d0)*sigR(j)
                if( pflag ) v2dt(:,imol) = v(:,imol) + dt_wmi2(atlist(j))*f(:,imol)
                if( fflag )then
                    fL(:,imol) =  dimlist(:,j)*&
                        ( - gamma(j)*v2dt(:,imol) + r83(:) )
                    f(:,imol) = f(:,imol) + fL(:,imol)
                end if
                ! ---
            end do
        end do
    end subroutine fix_langevin_LAMMPS
    !
    ! --- compute culumtive enerty ---
    subroutine fix_langevin_LAMMPS_compute
        use update_mod
        use commn
        use constant_mod
        use fix_compute_mod
    ...
    end subroutine fix_langevin_LAMMPS_compute
end module fix_langevin_LAMMPS_mod
